...
def make_recruit_embed(data):
    ...
    embed.add_field(
        name=f"🔥 딜러 모집  {len(data['dealer'])}/{data['max_dealer']}",
        value=make_member_text(data["dealer"]),
        inline=False
    )

    embed.add_field(
        name=f"💚 서포터 모집  {len(data['support'])}/{data['max_support']}",
    ...
    embed.add_field(
        name="📌 안내",
        value=(
            "• 신청은 등록된 원정대 캐릭터로만 가능합니다.\n"
            "• 출발 10분 전 신청자에게 알림이 전송됩니다.\n"
            "• 출발 시간이 지나면 자동으로 모집이 마감됩니다.\n"
            "• 캐릭터 변경은 다시 신청하면 자동 변경됩니다."
        ),
        inline=False
    )
...
async def reminder_task(message_id):
    ...
    wait_seconds = (
        data["start_time"] - timedelta(minutes=10) - datetime.now()
    ).total_seconds()
...
async def close_recruitment_task(message_id):
    ...
    wait_seconds = (data["start_time"] - datetime.now()).total_seconds()
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            await interaction.response.send_message(
                "날짜/시간 형식이 틀렸어요. 예: 2026-06-09 / 21:30",
                ephemeral=True
            )
            return
        ...
        await interaction.response.send_message(
            f"출발 시간 설정: {self.target_view.start_time_text}",
            ephemeral=True
        )
...
class RaidSetupView(discord.ui.View):
    ...
    @discord.ui.select(
        placeholder="레이드 선택",
        options=[discord.SelectOption(label=r) for r in RAIDS]
    )
    ...
    @discord.ui.select(
        placeholder="난이도 선택",
        options=[discord.SelectOption(label=d) for d in DIFFICULTIES]
    )
    ...
    @discord.ui.select(
        placeholder="숙련도 선택",
        options=[discord.SelectOption(label=s) for s in SKILLS]
    )
...
class EditRecruitView(discord.ui.View):
    ...
    async def apply_edit(self, interaction, button):
        ...
        if self.raid:
            ...
            data["dealer"] = data["dealer"][:data["max_dealer"]]
            data["support"] = data["support"][:data["max_support"]]
        ...
        if self.start_time:
            ...
            data["start_time_text"] = self.start_time_text
            data["closed"] = False

        if data.get("reminder"):
            data["reminder"].cancel()

        if data.get("close_task"):
...
class JoinView(discord.ui.View):
    ...
    async def edit_recruitment(self, interaction, button):
        ...
        await interaction.response.send_message(
            "변경할 항목만 선택한 뒤 수정 적용을 눌러주세요.",
            view=EditRecruitView(self.message_id),
            ephemeral=True
        )
